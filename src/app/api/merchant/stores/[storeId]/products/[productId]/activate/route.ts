...
import { activateProduct } from "@/modules/products/controller/products.controller";
  ...
  return withErrorHandling(
    withAuth(async (req: NextRequest, context) => {
      ...
      if (context.user.type !== "merchant") {
        throw ApiError.forbidden("Apenas lojistas podem ativar produtos");
      }

      // Resolver params se for Promise (Next.js 15+)
      const resolvedParams = params instanceof Promise ? await params : params;

      return activateProduct(context.user.id, resolvedParams.storeId, resolvedParams.productId, req);
    }),
  )(request, {});
}
