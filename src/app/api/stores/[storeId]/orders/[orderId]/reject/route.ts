...
import { rejectOrder } from "@/modules/orders/controller/orders.controller";
  ...
  return withErrorHandling(
    withAuth(async (req: NextRequest, context) => {
      ...
      if (context.user.type !== "merchant") {
        throw ApiError.forbidden("Apenas lojistas podem rejeitar pedidos");
      }
      ...
      try {
        return await rejectOrder(req, resolvedParams.orderId, context.user.id);
      } catch (error) {
