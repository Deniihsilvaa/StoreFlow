...
import { confirmOrder } from "@/modules/orders/controller/orders.controller";
  ...
  return withErrorHandling(
    withAuth(async (req: NextRequest, context) => {
      ...
      if (context.user.type !== "merchant") {
        throw ApiError.forbidden("Apenas lojistas podem confirmar pedidos");
      }
      ...
      try {
        return await confirmOrder(req, resolvedParams.orderId, context.user.id);
      } catch (error) {
        if (error instanceof Error) {
          ...
          if (error.message === "PEDIDO_JA_PROCESSADO") {
            throw ApiError.conflict("Este pedido já foi processado");
          }
