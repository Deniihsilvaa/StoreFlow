...
class PrismaMCPServer {
  ...
  setupInitializeHandler() {
    // Handler de inicialização obrigatório do MCP
    this.server.setRequestHandler('initialize', async (request) => {
      ...
      console.error('MCP Server: initialize chamado', JSON.stringify(response, null, 2));
      return response;
    });

    // Handler de notificação de inicialização concluída
    this.server.setNotificationHandler('notifications/initialized', async () => {
      // Servidor pronto para receber requisições
      console.error('MCP Server: inicializado e pronto');
    });
  }
...
const isMainModule = process.argv[1] && process.argv[1].endsWith('prisma-mcp-server.mjs');
